...
public class Helped_person extends AppCompatActivity {
    ...
    void setEventHandler() {
        engine.setEventHandler(new IZegoEventHandler() {
            ...
            public void onPublisherStateUpdate(String streamID, ZegoPublisherState state, int errorCode, JSONObject extendedData) {
                ...
                if (errorCode != 0) {
                    Log.d(TAG, "onPublisherStateUpdate: 推流状态出错");
                    //推流状态出错
                }
                if (state == ZegoPublisherState.PUBLISHING) {
                    ...
                    //未推流
                    Log.d(TAG, "onPublisherStateUpdate: 没有推流");
            ...
            public void onPlayerStateUpdate(String streamID, ZegoPlayerState state, int errorCode, JSONObject extendedData) {
                ...
                if (errorCode != 0) {
                    Log.d(TAG, "onPublisherStateUpdate: 拉流状态出错");
                    //拉流状态出错
                }
                if (state == ZegoPlayerState.PLAYING) {
                    //正在拉流中
                    Log.d(TAG, "onPublisherStateUpdate: 正在拉流中");
                } else if (state == ZegoPlayerState.NO_PLAY){
                    //未拉流
                    Log.d(TAG, "onPublisherStateUpdate: 没有拉流");
                } else if (state == ZegoPlayerState.PLAY_REQUESTING){
                    //正在请求拉流中
                    Log.d(TAG, "onPublisherStateUpdate: 正在请求拉流中");
                }
            }
